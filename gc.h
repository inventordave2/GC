...
extern volatile struct GC_* getGC();

// Sets the Active GC context. This is also done implicitly on a load-time call to fnc: initGC(int).
// Returns 1 on SUCCESS, 0 on FAILURE.
// It will fail if the GC_* structure is not initialised properly. Use fnc: build_gc_struct( int ) to initialize a GC structure.
extern int setGC( volatile struct GC_* );
...
extern int realloc_gc( volatile struct GC_* _, int c );
